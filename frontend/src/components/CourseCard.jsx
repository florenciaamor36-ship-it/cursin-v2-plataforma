  ...
  return (
    <>
      ...
      >
        ...
        <div className="p-4 md:p-6 flex flex-col flex-grow text-white">
          ...
          <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between text-[9px] font-bold text-zinc-400 uppercase tracking-[0.2em]">
            <span>{provider}</span>
            <span className="text-yellow-600 font-black shrink-0">+ INFO</span>
          </div>
        </div>
      </div>
      ...
      {isModalOpen && (
        <div 
          className="fixed inset-0 bg-black/98 backdrop-blur-xl flex items-start md:items-center justify-center p-0 md:p-6 z-[9999999]" 
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh' }}
          onClick={() => setIsModalOpen(false)}
        >
          <div 
            className="bg-zinc-950 w-full h-full md:h-auto md:max-w-5xl md:max-h-[90vh] overflow-y-auto relative shadow-2xl border-white/5 md:border"
            onClick={(e) => e.stopPropagation()}
          >
            <button 
              onClick={() => setIsModalOpen(false)}
              className="fixed top-4 right-4 md:absolute md:top-8 md:right-8 bg-black/60 text-white p-3 rounded-full hover:bg-yellow-600 hover:text-black transition-all z-[10000000]"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
            <div className="flex flex-col md:flex-row min-h-full text-white">
              <div className="md:w-1/2 bg-zinc-900 border-b md:border-b-0 md:border-r border-white/5">
              ...
              <div className="md:w-1/2 p-6 md:p-12 flex flex-col justify-center">
                <div className="flex flex-wrap items-center gap-2 mb-6">
                  ...
                  <span className="text-zinc-500 font-mono text-[9px] border border-white/10 px-2 py-1 uppercase tracking-tighter text-white">PROTOCOLO LCA v5.0</span>
                </div>
