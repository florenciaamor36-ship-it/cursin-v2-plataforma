...
const CourseCard = ({ course, isFavorite, toggleFavorite }) => {
  ...
  const rating = useMemo(() => {
    return (course.Title || course.title || "").length % 2 === 0 ? 5 : 4;
  ...
  const getProviderImage = (prov, cat, tit) => {
    ...
    if (c.includes('programación') || c.includes('it') || c.includes('software') || t.includes('python')) return 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?q=80&w=800&auto=format&fit=crop';
    if (c.includes('marketing') || c.includes('negocios')) return 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=800&auto=format&fit=crop';
  ...
  return (
    <>
      <div onClick={() => setIsModalOpen(true)} className="group relative bg-zinc-900/40 border border-white/10 overflow-hidden transition-all hover:border-yellow-600 flex flex-col h-full cursor-pointer shadow-lg active:scale-95 md:active:scale-100">
        <div className="aspect-video w-full overflow-hidden bg-zinc-900 flex items-center justify-center p-0 relative">
          ...
          <button onClick={(e) => { e.stopPropagation(); toggleFavorite(link); }} className="absolute top-4 left-4 z-10 p-2 bg-black/40 backdrop-blur-md rounded-full border border-white/10 hover:scale-110 transition-all">
            {isFavorite ? <span className="text-red-500 text-xl">❤️</span> : <span className="text-white text-xl">🤍</span>}
          </button>
        </div>
        ...
        <div className="fixed inset-0 bg-black/98 backdrop-blur-xl flex items-start md:items-center justify-center p-0 md:p-6 z-[9999999]" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0 }} onClick={() => setIsModalOpen(false)}>
          <div className="bg-zinc-950 w-full h-full md:h-auto md:max-w-5xl md:max-h-[90vh] overflow-y-auto relative shadow-2xl border-white/5 md:border" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col md:flex-row min-h-full text-white text-left">
              <div className="md:w-1/2 bg-zinc-900 border-b md:border-b-0 md:border-r border-white/5 flex items-center justify-center p-0 relative">
                <img src={image} alt={title} className="w-full h-full object-cover opacity-80" />
                <button onClick={() => toggleFavorite(link)} className="absolute bottom-6 right-6 p-4 bg-black/60 backdrop-blur-xl rounded-full border border-white/10 hover:scale-110 transition-all">
                  {isFavorite ? <span className="text-red-500 text-2xl">❤️</span> : <span className="text-white text-2xl">🤍</span>}
                </button>
              </div>
              <div className="md:w-1/2 p-6 md:p-12 flex flex-col justify-center">
                <div className="flex flex-wrap items-center gap-4 mb-6">
                  ...
                  <span className="text-zinc-500 font-mono text-[9px] border border-white/10 px-2 py-1 uppercase tracking-tighter">PROTOCOLO LCA v10.0</span>
                </div>
