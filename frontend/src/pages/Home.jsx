...
const Home = () => {
  ...
  const fetchChunk = async (index) => {
    try {
      // Usamos el path relativo correcto para GitHub Pages
  ...
  if (loading && allCourses.length === 0) return (
    <div className="min-h-screen bg-black flex items-center justify-center">
      <div className="text-yellow-600 font-mono text-xs uppercase animate-pulse text-center tracking-[0.5em]">Sincronizando Archivos Globales...</div>
    </div>
  );
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-yellow-600 selection:text-black overflow-x-hidden">
      ...
      <div className="container mx-auto px-6 pt-12 pb-12 md:pt-24 md:pb-24 text-left border-b border-white/5">
        <div className="inline-flex items-center gap-4 mb-8">
          <div className="bg-yellow-600 text-black px-3 py-1 text-[10px] font-black tracking-widest uppercase">LCA Universal Protocol v5.5</div>
          <div className="text-zinc-600 font-mono text-[8px] uppercase tracking-[0.5em] animate-pulse">Live // Updated</div>
        </div>
        ...
        <div className="mt-16 flex flex-wrap justify-start gap-2">
          {categories.map(cat => (
            <button 
              key={cat} 
              onClick={() => { setSelectedCategory(cat); setVisibleCount(100); }} 
              className={`px-5 py-2 text-[10px] font-bold uppercase tracking-widest border transition-all ${selectedCategory === cat ? 'bg-white text-black border-white' : 'bg-zinc-900 text-zinc-500 border-white/5 hover:border-white/20'}`}
            >
              {cat}
            </button>
          ))}
      ...
      <div className="bg-zinc-950 sticky top-0 z-50 border-b border-white/5 backdrop-blur-xl bg-black/80">
        <div className="container mx-auto px-6 py-4 md:py-8">
          <input 
            type="text" 
            placeholder="BUSCAR POR UNIVERSIDAD, PAÍS, PROVINCIA O TEMA..." 
            value={searchQuery} 
            onChange={(e) => { setSearchQuery(e.target.value); setVisibleCount(100); }} 
            className="w-full bg-transparent border-b-2 border-zinc-800 py-4 md:py-6 text-xl md:text-4xl font-black uppercase tracking-tighter focus:outline-none focus:border-yellow-600 transition-colors text-white placeholder:text-zinc-900" 
          />
        </div>
      </div>
        ...
        {(visibleCount < filteredCourses.length || hasMoreChunks) && (
          <div className="flex justify-center mt-24">
            <button 
              ...
              className="px-12 py-5 border border-yellow-600 text-yellow-600 font-black uppercase tracking-widest hover:bg-yellow-600 hover:text-black transition-all text-xs"
            > 
      ...
      <footer className="py-24 border-t border-white/5 text-center">
        <p className="text-zinc-600 font-mono text-[10px] tracking-[0.5em] uppercase">LCA // GLOBAL DATA // 2026</p>
      </footer>
    </div>
  );
};
