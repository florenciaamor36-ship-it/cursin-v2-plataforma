    ...
    const init = async () => {
      ...
      for (let i = 0; i < 90; i += batchSize) {
        ...
        for (let j = 1; j <= batchSize; j++) {
          if (i + j <= 90) promises.push(fetchChunk(i + j));
        }
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-yellow-600 selection:text-black overflow-x-hidden">
      <div className="container mx-auto px-6 pt-12 pb-12 md:pt-24 md:pb-24 text-left border-b border-white/5">
        <div className="inline-flex items-center gap-4 mb-8">
          <div className="bg-yellow-600 text-black px-3 py-1 text-[10px] font-black tracking-widest uppercase">LCA Massive Protocol v6.5</div>
          <div className="text-zinc-600 font-mono text-[8px] uppercase tracking-[0.5em] animate-pulse">Sincronizado: {allCourses.length} cursos</div>
        </div>
